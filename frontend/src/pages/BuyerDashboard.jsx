...
const CONTRACT_ADDRESS = "0x8b7aFada8350844ae24049F98fcF2397D54E6cfe"; // Replace with your deployed contract address
const CONTRACT_ABI = [
    {
      "anonymous": false,
      "inputs": [
...
function BuyerDashboard({ account }) {
    ...
    // Helper function to format ETH price from wei with INR
    const formatEthPrice = (weiPrice) => {
        try {
            const eth = window.web3.utils.fromWei(weiPrice.toString(), "ether");
        ...
        if (bidEnabled && selectedCrop) {
            try {
                ...
                if (bidPriceNum < highestBidAmount) {
                    setSnackbar({
                        open: true,
                        message: `Your bid (${formatEthInr(bidPriceNum.toFixed(4), ethInrRate)}) is too low. Minimum bid is ${formatEthInr(highestBidAmount.toFixed(4), ethInrRate)}`,
                        severity: 'error'
                    });
                    return;
                }
                ...
                if (bidPriceNum < minimumBidNum) {
                    setSnackbar({
                        open: true,
                        message: `Your bid (${formatEthInr(bidPriceNum.toFixed(4), ethInrRate)}) is too low. Minimum bid is ${formatEthInr(minimumBidNum.toFixed(4), ethInrRate)}`,
                        severity: 'error'
                    });
                    return;
                }
    ...
    const getAcceptedCustomBidPrice = (requestId) => {
        try {
            ...
            if (customBid && customBid.customPriceWei) {
                console.log(`Found custom bid price for request ${requestId}: ${formatEthInr(customBid.customPrice, ethInrRate)}`);
                return {
    ...
    const completePurchase = async (requestId, priceInWei, quantity) => {
        try {
            ...
            if (customBid && customBid.customPriceWei) {
                ...
                priceExplanation = `custom bid price (${formatEthInr(customBid.customPriceEth, ethInrRate)})`;
                console.log(`Using custom bid price of ${formatEthInr(customBid.customPriceEth, ethInrRate)} instead of original price`);
            }
            ...
            try {
                ...
                setSnackbar({
                    open: true,
                    message: customBid 
                        ? `Purchase completed successfully at custom bid price of ${formatEthInr(customBid.customPriceEth, ethInrRate)}!`
    ...
    const analyzeDeliveryIssues = async () => {
        try {
            ...
            // Get escrow balance
            const escrowBalance = await contract.methods.escrowBalances(localAccount).call();
            console.log("Escrow balance:", escrowBalance, "wei", web3.utils.fromWei(escrowBalance, 'ether'), "ETH", formatEthInr(web3.utils.fromWei(escrowBalance, 'ether'), ethInrRate));
            
            // Check account balance
            const accountBalance = await web3.eth.getBalance(localAccount);
            console.log("Account balance:", accountBalance, "wei", web3.utils.fromWei(accountBalance, 'ether'), "ETH", formatEthInr(web3.utils.fromWei(accountBalance, 'ether'), ethInrRate));
            ...
            if (pendingDeliveries.length > 0) {
                ...
                for (let i = 0; i < pendingDeliveries.length; i++) {
                    ...
                    console.log("Amount held in escrow:", web3.utils.fromWei(delivery.amountHeld, 'ether'), "ETH", formatEthInr(web3.utils.fromWei(delivery.amountHeld, 'ether'), ethInrRate));
    ...
    const handlePlaceBid = async () => {
      ...
      try {
        ...
        if (bidAmountNum < minimumBidNum) {
          setSnackbar({
            open: true,
            message: `Your bid (${formatEthInr(bidAmountNum.toFixed(4), ethInrRate)}) is too low. Minimum bid is ${formatEthInr(minimumBidNum.toFixed(4), ethInrRate)}`,
            severity: 'error'
          });
          return;
        }
...
return (
        <Box sx={{ 
                ...
                <Box sx={{ 
                    ...
                    <Box sx={{ 
                        ...
                        <Box sx={{ 
                            ...
                            {['Available Crops', 'My Requests', 'My Purchases'].map((tab, index) => (
                                <Box
                                    key={index}
                            ...
                            >
                                ...
                                >
                                    ...
                                    <Typography variant="body2" sx={{ 
                                        ...
                                        {buyerName || (localAccount ? `${localAccount.substring(0, 6)}...${localAccount.substring(localAccount.length - 4)}` : 'Not Connected')}
                                    </Typography>
                                </Box>
                            </motion.div>
            ...
            <Box sx={{ 
                ...
                {tabValue === 0 && (
                    // Available Crops Tab - Premium Cards
                    <motion.div 
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ duration: 0.5, delay: 0.3 }}
                    >
                        {/* Welcome Section with Search - Only in Available Crops */}
                        <motion.div 
                                    ...
                                    <Box sx={{ 
                                        ...
                                        <Box>
                                            ...
                                            <Typography variant="body1" sx={{ 
                                                ...
                                                Discover and purchase high-quality crops directly from verified farmers
                                            </Typography>
                                        </Box>
                                                ...
                                                <Box sx={{ p: 4, flexGrow: 1 }}>
                                                        ...
                                                        {crop && cropHighestBids && cropHighestBids[crop.id] && (
                                                            <Box sx={{ textAlign: 'right' }}>
                                                                ...
                                                                <Typography variant="h6" sx={{ 
                                                                    ...
                                                                    {formatEthInr(cropHighestBids[crop.id].amount, ethInrRate)}
                                                                </Typography>
                                                            </Box>
                                                        )}
                                                    </Box>
                                                    ...
                                                    <Box sx={{ 
                                                        ...
                                                        <Box sx={{ 
                                                            ...
                                                            <Typography variant="h6" sx={{ 
                                                                ...
                                                                #{crop.cropID ? crop.cropID.toString() : crop.id}
                                                            </Typography>
                                                        </Box>
                                                    </Box>
                                                    ...
                                                    <Button 
                                                        ...
                                                        sx={{
                                                            py: 2,
                                                            borderRadius: '16px',
                                                            background: parseInt(crop.quantity) === 0 
                                                            ...
                                                            fontSize: '16px',
                                                            textTransform: 'none',
                                                            boxShadow: parseInt(crop.quantity) === 0 
                                                                ? 'none'
                                                                : '0 8px 25px rgba(102, 126, 234, 0.3)',
                                                            '&:hover': parseInt(crop.quantity) === 0 ? {} : {
                            ...
                            <Box sx={{ 
                                ...
                                <Box sx={{ position: 'relative', zIndex: 1 }}>
                                    ...
                                    <Typography variant="body1" sx={{ 
                                        ...
                                        Track and manage your crop purchase requests
                                    </Typography>
                                </Box>
                            </Box>
                        </motion.div>
                                    ...
                                    {purchaseRequests.length > 0 ? (
                                        purchaseRequests.map((request, index) => (
                                            ...
                                            >
                                                ...
                                                <TableCell>{formatEthPrice(request.price)} per kg</TableCell>
                                                <TableCell>
                                                    <Chip 
                ...
                {tabValue === 2 && (
                    // My Purchases Tab - Premium Cards
                    <motion.div 
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ duration: 0.5, delay: 0.3 }}
                            ...
                            <Box sx={{ 
                                ...
                                <Box sx={{ position: 'relative', zIndex: 1 }}>
                                    ...
                                    <Typography variant="body1" sx={{ 
                                        ...
                                        View and manage your completed crop purchases and deliveries
                                    </Typography>
                                </Box>
                            </Box>
                        </motion.div>
                                    ...
                                    {buyerOrders.length > 0 ? (
                                        buyerOrders.map((order, index) => (
                                            <TableRow key={index}>
                                                ...
                                                <TableCell>
                                                    {(() => {
                                                        const ethAmount = order.priceInEth || order.priceEth || order.amountHeldEth;
                                                        if (!ethAmount || ethAmount === "Unknown") return "Unknown";
                                                        return formatEthInr(ethAmount, ethInrRate);
                                                    })()}
                                                </TableCell>
                                                <TableCell>
                                                    <Chip 
                                                ...
                                                <TableCell>
                                                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                                                        ...
                                                        {(order.orderType === 'pending' || order.statusText === 'Awaiting Delivery' || order.isDelivered === false) && (
                                                            <Button
                                                                variant="contained"
                                                                color="success"
                                                                size="small"
                                                                onClick={() => confirmDelivery(order.cropID)}
                                                            >
                                                                Confirm Delivery
                                                            </Button>
                                                        )}
                                                        {/* Also show when farmer marked delivered (soft signal via localStorage) */}
                                                        {(() => {
                                                            try {
                                                                const marks = JSON.parse(localStorage.getItem('farmerMarkedDelivered') || '[]');
                                                                if (marks.includes(order.cropID?.toString())) {
                                                                    return (
                                                                        <Button
                                                                            variant="contained"
                                                                            color="success"
        ...
        {/* Buy Crop Dialog - Premium Shadcn Style */}
        <Dialog 
            open={showModal} 
            onClose={closeModal}
            maxWidth="lg"
            ...
            <Box sx={{ 
                ...
                <Box sx={{ position: 'relative', zIndex: 1 }}>
                    ...
                    <Typography variant="body1" sx={{ 
                        ...
                        Submit your purchase request to the farmer
                    </Typography>
                </Box>
            </Box>
            <DialogContent sx={{ 
                ...
                {selectedCrop && (
                    <Grid container spacing={4}>
                        ...
                        <Grid item xs={12} md={selectedCrop.imageCID ? 7 : 12}>
                            <Box sx={{ mb: 3 }}>
                                <Typography variant="h5" sx={{ 
                                    fontWeight: 700,
                                    color: theme === 'dark' ? '#ffffff' : '#1f2937',
                                ...
                                <Box sx={{ 
                                    ...
                                    <Typography variant="body2" sx={{ 
                                        ...
                                        📅 Delivery: {selectedCrop.deliveryDate}
                                    </Typography>
                                    <Typography variant="body2" sx={{ 
                                        color: theme === 'dark' ? '#94a3b8' : '#64748b',
                                        fontWeight: 500
                                    }}>
                                        👨‍🌾 Farmer: {selectedCrop.farmerName || selectedCrop.farmer}
                                    </Typography>
                                </Box>
                            </Box>
                        </Grid>
                            ...
                            <Box sx={{ 
                                    ...
                                    <Grid item xs={12} md={6}>
                                        <Box sx={{ mb: 1 }}>
                                            ...
                                            <Typography variant="caption" sx={{ 
                                                ...
                                                📈 Max available: {selectedCrop.quantity} kg
                                            </Typography>
                                        </Box>
                                    </Grid>
                                    <Grid item xs={12} md={6}>
                                        ...
                                        <Box sx={{ mt: 3 }}>
                                            {selectedCrop && cropHighestBids && cropHighestBids[selectedCrop.id] ? (
                                                <Box sx={{ 
                                                    ...
                                                    <Typography variant="body2">
                                                        Highest bid: <strong>{parseFloat(cropHighestBids[selectedCrop.id].amount).toFixed(4)} ETH</strong> by {getBuyerDisplayName(cropHighestBids[selectedCrop.id].bidder)}
                                                ...
                                                <Box sx={{ 
                                                    ...
                                                    <Typography variant="body2">
                                                        No bids yet. Minimum: {formatEthPrice(selectedCrop?.price)} (base price)
                                                    </Typography>
                                                </Box>
                                            )}
                                                ...
                                                onBlur={() => {
                                                  if (bidPrice && !isNaN(parseFloat(bidPrice)) && selectedCrop) {
                                                    ...
                                                    if (bidAmountNum < minimumBidNum) {
                                                      setSnackbar({
                                                        open: true,
                                                        message: `Your bid must be at least ${formatEthInr(minimumBidNum.toFixed(4), ethInrRate)}`,
                                                        severity: 'warning'
                                                      });
                                                    }
                                                  }
                                ...
                                <Box sx={{ mt: 3 }}>
                                    ...
                                    <TextField
                                        ...
                                        placeholder="Why are you interested in this crop? Any special requirements or questions?"
                                        sx={{
                                            '& .MuiOutlinedInput-root': {
                                                borderRadius: '16px',
                                                bgcolor: theme === 'dark' ? '#0f0f0f' : '#ffffff',
                                    ...
                                    >
                                        <Box sx={{ 
                                            ...
                                            <Typography variant="h4" sx={{ 
                                                ...
                                                {bidEnabled && bidPrice 
                                                    ? formatEthInr((parseFloat(bidPrice) * parseFloat(purchaseQuantity)).toFixed(4), ethInrRate)
                                                    : formatEthInr((parseFloat(formatEthPrice(selectedCrop.price)) * parseFloat(purchaseQuantity)).toFixed(4), ethInrRate)}
                                            </Typography>
                                            <Typography variant="body2" sx={{ 
                                                opacity: 0.9,
                                                fontWeight: 500
                                                ...
                                                {bidEnabled 
                                                    ? "🎯 Your custom bid will be sent to the farmer for consideration"
                                                    : "📋 Estimate based on base price. Final payment after farmer approval"}
                                            </Typography>
                                        </Box>
                                    </motion.div>
                                )}
            ...
            <Box sx={{ 
                ...
                <Button 
                    ...
                    sx={{
                        ...
                        textTransform: 'none',
                        fontSize: '16px',
                        background: (!purchaseQuantity || loading || (bidEnabled && (!bidPrice || isNaN(parseFloat(bidPrice)) || parseFloat(bidPrice) <= 0))) 
                            ? 'linear-gradient(135deg, #6b7280 0%, #4b5563 100%)'
                            : 'linear-gradient(135deg, #10b981 0%, #059669 100%)',
                        boxShadow: (!purchaseQuantity || loading) 
                            ? 'none'
                            : '0 8px 25px rgba(16, 185, 129, 0.4)',
                        '&:hover': (!purchaseQuantity || loading) ? {} : {
                ...
                {selectedCrop && (
                    <>
                        <Typography variant="body1" gutterBottom>
                            Base Price: {formatEthPrice(selectedCrop.price)}
                        </Typography>
                        <Typography variant="body1" gutterBottom>
                        ...
                        {cropHighestBids && cropHighestBids[selectedCrop.id] && (
                            <Box sx={{ 
                                ...
                                <Typography 
                                    ...
                                    sx={{ fontWeight: 'bold' }}
                                >
                                    Current Highest Bid: {formatEthInr(parseFloat(cropHighestBids[selectedCrop.id].amount).toFixed(4), ethInrRate)}
                                </Typography>
                        ...
                        <TextField
                            ...
                            helperText={
                              ...
                              ? `Bid too low! Minimum: ${
                                selectedCrop && cropHighestBids && cropHighestBids[selectedCrop.id] 
                                ? formatEthInr(cropHighestBids[selectedCrop.id].amount, ethInrRate)
                                : formatEthPrice(selectedCrop.price)
                              }` 
                              : selectedCrop && cropHighestBids && cropHighestBids[selectedCrop.id] 
                              ? `Minimum bid: ${formatEthInr(cropHighestBids[selectedCrop.id].amount, ethInrRate)} (current highest bid)` 
                              : `Minimum bid: ${formatEthPrice(selectedCrop?.price)} (base price)`
                            }
